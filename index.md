...
**Developer contact:** _your-email@example.com_ ← replace before publishing
...
Lakshya does not send data to Google directly; it only marks certain files as backup-eligible per [Android's backup rules](https://developer.android.com/guide/topics/data/autobackup). The backup rules are defined in `res/xml/backup_rules.xml` in the app's source.
...
For privacy questions, write to _your-email@example.com_.

Source: [github.com/Rushil-s/lakshya](https://github.com/Rushil-s/lakshya)
